...
const Content = () => {
    ...
    useEffect(() => {
        ;(async () => {
            ...
            const data = (await fetch(url + `&page=${page}`).then((res) =>
                res.json()
            )) as MovieDBResponse
            ...
            if (
                ...
                'poster_path' in columnVisibilityModel
            ) {
                setRowHeight(
                    columnVisibilityModel.poster_path ? maxHeight : minHeight
                )
            }
        })()
    }, [])
    ...
    const handleChangePage = async (model: GridPaginationModel) => {
        const data = (await fetch(
            url + `&page=${Number(model?.page) + 1}`
        ).then((res) => res.json())) as MovieDBResponse
        setMovies(data.results)
